...
class BPA:
    def __init__(self, path, radius,  num_workers=1):
        ...
        self.radius = radius
        self.cell = Cell(points=self.points, radius=radius)
    ...
    @staticmethod
    def get_points_distances_from_edge(points: List, p1: Point, p2: Point):
        
        dists_p1 = [round(utility.calc_distance_points(p1, p3), 2) for p3 in points]
        dists_p2 = [round(utility.calc_distance_points(p2, p3), 2) for p3 in points]
        dists = [round(dists_p1[i] + dists_p2[i], 2) for i in range(len(dists_p1))]
        return dists

    @staticmethod
    def get_third_point_of_triangle(triangle_edges: List, p1: Point, p2: Point):
        triangle_points = []
        third_point = None
    ...
    @staticmethod
    def will_triangles_overlap(edge: Edge, p3: Point, p4: Point):
    ...
    def create_mesh(self, first_point_index: int = 0):
    ...
    def find_seed_triangle(self, first_point_index=0, num_recursion_calls=0):
        ...
        for cell in p1.neighbor_nodes:
            p1_neighbor_points.extend(self.cell.get_cell_points(cell))

        p1_neighbor_points = set(p1_neighbor_points)

        # Sort points by distance from p1.
        dists = [utility.calc_distance_points(p1, p2) for p2 in p1_neighbor_points]
        p1_neighbor_points = [x for _, x in sorted(zip(dists, p1_neighbor_points))]

        # If we couldn't find a close point to expand to, it's better just to find new
        ...
        for p2 in p1_neighbor_points:
            ...
            for cell in intersect_cells:
                possible_points.extend(self.cell.get_cell_points(cell))

            # Sort points by distance from p2.
            dists_p2 = [utility.calc_distance_points(p2, p3) for p3 in possible_points]
            dist_p1 = [utility.calc_distance_points(p1, p3) for p3 in possible_points]
            ...
            for i, p3 in enumerate(possible_points):
                ...
                # triangle.
                if self.radius <= utility.calc_incircle_radius(p1, p2, p3):
                    ...
                    p1_and_p3_already_connected = [e for e in self.cell.edges if ((e.p1.id == p1.id)
                                                                                  and (e.p2.id == p3.id)) or (
                                                               (e.p1.id == p3.id)
                                                               and (e.p2.id == p1.id))]
                    p1_and_p2_already_connected = [e for e in self.cell.edges if ((e.p1.id == p1.id)
                                                    and (e.p2.id == p2.id)) or ((e.p1.id == p2.id)
                                                    and (e.p2.id == p1.id))]
                    p2_and_p3_already_connected = [e for e in self.cell.edges if ((e.p1.id == p2.id)
                    ...
                    min_angle, max_angle = utility.calc_min_max_angle_of_triangle(e1, e2, e3)
                    if max_angle > 170 or min_angle < 20:
                        continue

                    self.cell.edges.append(e1)
                    self.cell.edges.append(e2)
                    self.cell.edges.append(e3)

                    triangle = sorted(list({e1.p1, e1.p2, e2.p1, e2.p2, e3.p1, e3.p2}))
                    self.cell.triangles.append(triangle)
    ...
    def expand_triangle(self, edge: Edge, triangle_edges: List[Edge]):
        if edge.num_triangles_this_edge_is_in < 2:
            ...
            for index, p3 in enumerate(sorted_possible_points):
                ...
                # the triangle.
                t = utility.calc_incircle_radius(p1, p2, p3)
                if self.radius <= utility.calc_incircle_radius(p1, p2, p3):
                    ...
                    min_angle, max_angle = utility.calc_min_max_angle_of_triangle(e1, e2, edge)
                    if max_angle > 180 or min_angle < 1:
                        continue

                    self.cell.add_edge(e1)
                    self.cell.add_edge(e2)
    ...
    def find_triangles_by_edge(self, edge: Edge):
        possible_triangles = []

        for triangle in self.cell.triangles:
